...
class Money(commands.Cog, name='경제'):
    ...
    @commands.command(name='포인트', help='자신이 가지고 있는 돈을 보여줍니다.', usage='<유저 닉네임 또는 멘션>')
    ...
    @commands.command(name='도박', aliases=['베팅'], help='포인트를 걸고 도박을 합니다.', usage='[걸 포인트]')
    @can_use()
    ...
    async def _betting(self, ctx, amount):
        ...
        if result == 1:
            ...
            await sendEmbed(ctx=ctx, title='휴...', content='돈은 잃지는 않았네요!')
        else:
            writedata(id=ctx.author.id, item='point', value=str(point-amount))
            await sendEmbed(ctx=ctx, title='이런!', content=f'아쉽게 져서 💵 {amount}만큼 잃었어요...\n현재 포인트: `{point-amount}`')
    ...
    async def _sendmoney(self, ctx, user:discord.User, amount:int):
        if amount < 100:
            await warn(ctx=ctx, content='100원 이상부터 송금할 수 있습니다')
            return
        if user.bot:
